...
#Box Plots
p1 <- ggplot(alien, aes(x = event, y = - oxygen/(end_weight - start_weight), fill = stimuli)) +
  geom_boxplot() +
  labs(x = "Event", y = "Oxygen consumption per weight loss in ml/gr", fill = "Stimuli") +
  theme_bw() +
  ...
  theme(panel.grid.major.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.text = element_text(size = 12, color = "black"), axis.title = element_text(size = 18))
ggsave(paste0(getwd(), "/graphs/boxplot_alien.pdf"), p1)
p2 <- ggplot(familiar, aes(x = event, y = - oxygen/(end_weight - start_weight), fill = stimuli)) +
  geom_boxplot() +
  labs(x = "Event", y = "Oxygen consumption per weight loss in ml/gr", fill = "Stimuli") +
  theme_bw() +
...
#Point + Line
alien$mean_o <- ave(alien$oxygen/(alien$start_weight - alien$end_weight), alien$time)
familiar$mean_o <- ave(familiar$oxygen/(familiar$start_weight - familiar$end_weight), familiar$time)
...
p3 <- ggplot(alien, aes(x = time, y = mean_o)) +
  ...
  labs(x = "Time in s", y = "Oxygen consumption per weight loss in ml/gr", fill = "Stimuli", title = "Alien species") +
  theme_bw() +
  scale_y_continuous(limits = c(4.5, 6), expand = c(0, 0)) +
...
p4 <- ggplot(familiar, aes(x = time, y = mean_o)) +
  ...
  geom_smooth(size = 2, color = "#1f78b4") +
  geom_vline(xintercept = alien$time[time_cuts], size = 1, linetype = "dashed") +
  labs(x = "Time in s", y = "Oxygen consumption per weight loss in ml/gr", fill = "Stimuli", title = "Familiar species") +
  theme_bw() +
  scale_y_continuous(limits = c(4.5, 6), expand = c(0, 0)) +
  scale_x_continuous(limits = c(0, 2400), expand = c(0, 0)) +
  theme(panel.grid.major.x = element_blank(), axis.text = element_text(size = 12), axis.title = element_text(size = 18), plot.title = element_text(size = 24))
